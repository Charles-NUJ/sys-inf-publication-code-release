...
# 
# include("scripts/evaluations/top_e_src_inf_analysis_runner_inf_nc.jl")
...
include("$(scriptsdir())/evaluations/top_e_src_inf_graph_analysis_inf_nc.jl")
...
function run(graph_sizes, force_calculation, graph_types, generate_random_topologies, heuristic, inf_enable, num_random_graphs = 1)
	for graph_type in graph_types
		...
		for graph_size in graph_sizes # Number of nodes
			...
			max_coding_degrees = [graph_size] # 最大编码度, 表示节点本地一次可以传输的包中, 含有的信息量
			max_inflights = [graph_size] # 最大飞行量, 表示网络中正在传输但是还没有所有节点都收到的信息个数
			...
			if enable_run
				...
				elseif heuristic[1] == true
					...
					res = tmpsave(params, projectdir(tmp_dir))
					len = length(res) #1-510
					for r in res#[51:len]
						config = load(projectdir(tmp_dir, r), "params")
						if config["graph_size"]==6 && (config["graph_id"]>=51) && config["graph_id"]<=100
...
# graph_sizes = [20]#,16]#,18,20]  v1跑了 8 10 12 和 14的一点点 先当作结果
