...
function pl_graphsize_degree_gain(graph_sizes, group_by, subresult)
    ...
    elseif group_by == "cutoff"
        bins = MAX_cutoff_desity:-0.05:0.2
        # bins = 0.5:-0.05:0.2
    ...
    for (i,graph_size) in enumerate(graph_sizes)
        ...
        for gdf in gdfs # 同一个cuttoff下
            ...
            gains = [(sys_df.aoi_from_direct[i][2]-nc_df.aoi_from_direct[i][2])/nc_df.aoi_from_direct[i][1] for i = 1:max_common_sample]
    ...
    label_set = string.(collect(MAX_cutoff_desity:-0.05:0.2))
    # label_set = string.(collect(0.5:-0.05:0.2))
    label_set[2:2:end] .= ""
    ax.set_yticks((1:1:y_dim) .-1, labels=label_set, fontsize = 12)#,fontfamily  = "Times New Roman")
    # ax.set_yticks((1:1:y_dim) .-1, labels=[], fontsize = 12)#,fontfamily  = "Times New Roman")
    ...
    # title("\$\\mathcal{T}~[\\%]\$", fontsize = 12)#,fontfamily  = "Times New Roman")
    title("\$\\mathcal{T}\$", fontsize = 12)#,fontfamily  = "Times New Roman")
    savefig(plotsdir("inf-sys-sysinf-graphsize_degree_gain-$save_title_string-$group_by-big-T.png"),bbox_inches="tight")
    savefig(plotsdir("inf-sys-sysinf-graphsize_degree_gain-$save_title_string-$group_by-big-T.eps"),bbox_inches="tight")
end
